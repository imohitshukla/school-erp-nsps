...
const FeePayment = () => {
  ...
  // ─── Generate receipt HTML as pure string and print via popup ───
  const printReceiptHTML = useCallback((data) => {
    if (!data) return;
    ...
    const remaining   = parseFloat(data.remaining_due !== undefined ? data.remaining_due : Math.max(0, netPayable - paidAmt));
    ...
    const feeRows = feeHeads.map((h, i) =>
      `<tr><td style="text-align:center;border:1px solid #ccc;padding:8px">${i+1}</td>
            <td style="border:1px solid #ccc;padding:8px">${h.label}</td>
            <td style="text-align:right;border:1px solid #ccc;padding:8px">${fmtR(h.v)}</td></tr>`
    ).join('');

    const summaryRows = [
      totalFeeDue > 0 ? `<tr style="font-weight:bold;background:#f0f0f0"><td colspan="2" style="border:1px solid #ccc;padding:8px">Total Fee Amount</td><td style="text-align:right;border:1px solid #ccc;padding:8px">${fmtR(totalFeeDue)}</td></tr>` : '',
      concession > 0  ? `<tr style="font-weight:bold;background:#f0f0f0"><td colspan="2" style="border:1px solid #ccc;padding:8px">Concession / Discount</td><td style="text-align:right;border:1px solid #ccc;padding:8px;color:#2563eb">(${fmtR(concession)})</td></tr>` : '',
      totalFeeDue > 0 ? `<tr style="font-weight:bold;background:#f0f0f0"><td colspan="2" style="border:1px solid #ccc;padding:8px">Net Payable</td><td style="text-align:right;border:1px solid #ccc;padding:8px">${fmtR(netPayable)}</td></tr>` : '',
      `<tr style="font-weight:bold;background:#f0fff4;color:#16a34a"><td colspan="2" style="border:1px solid #ccc;padding:8px">✅ Amount Paid</td><td style="text-align:right;border:1px solid #ccc;padding:8px">${fmtR(paidAmt)}</td></tr>`,
      `<tr style="font-weight:bold;background:#fff1f1;color:#dc2626"><td colspan="2" style="border:1px solid #ccc;padding:8px">🔴 Remaining Due</td><td style="text-align:right;border:1px solid #ccc;padding:8px">${fmtR(remaining)}</td></tr>`,
    ].join('');
...
<div class="wrap">
  <div class="header">
    ...
    <div class="school-info">
      ...
      <div class="school-sub">Affiliation: EJ-6/18-19 &nbsp;|&nbsp; UDISE: 09400405918 &nbsp;|&nbsp; Academic Year: 2026-2027</div>
    </div>
  ...
  <table>
    <thead>
      <tr><th style="width:50px">Sr.</th><th>Description</th><th class="right" style="width:180px">Amount (&#8377;)</th></tr>
    </thead>
    <tbody>
